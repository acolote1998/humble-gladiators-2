...
class BootsTemplateControllerTest {
    ...
    @Test
    @WithMockUser
    void getAllBootsTemplatesForACampaign_ShouldReturnListOfBoots() throws Exception {
        // Arrange
        Campaign campaign = new Campaign();
        campaign.setId(CAMPAIGN_ID);
        
        BootsTemplate bootsTemplate = new BootsTemplate();
        bootsTemplate.setId(100L);
        bootsTemplate.setName("Test Boots");
        bootsTemplate.setCampaign(campaign);
        List<BootsTemplate> boots = List.of(bootsTemplate);
        
        when(bootsService.getAllBootsTemplatesForACampaignAndUser(USER_ID, CAMPAIGN_ID)).thenReturn(boots);

        // Act & Assert
        mockMvc.perform(get("/api/campaign/{campaignId}/boots-templates", CAMPAIGN_ID)
                        .with(jwt().jwt(jwt))
    ...
    @Test
    @WithMockUser
    void getAllBootsTemplatesForACampaign_WhenEmpty_ShouldReturnEmptyList() throws Exception {
        // Arrange
        when(bootsService.getAllBootsTemplatesForACampaignAndUser(USER_ID, CAMPAIGN_ID))
