...
export const ShieldTemplateCard = ({
  ...
  return (
    <div
      className={`${renderingFromBooster && "rotate-y-180"} ${discovered && !renderingFromBooster && `cursor-zoom rarity-${rarity} ${discovered && !renderingFromBooster ? (tier == 5 && rarity == 5 ? `shield-tier-5-rarity-5` : `shield-tier-${tier}`) : ""}`} relative my-5 w-85 h-119 bg-cover bg-no-repeat p-2 select-none cursor-pointer `}
